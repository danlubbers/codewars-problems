...
function  calculateAge(birthYear, futureYear) {
    ...
    let willBeBorn = birthYear - futureYear;

    if(futureAge === 1) return `You are ${futureAge} year old.` 
    if(willBeBorn === 1) return `You will be born in ${willBeBorn} year.`
    if(birthYear === futureYear) return 'You were born this very year!' 
    if(birthYear < futureYear) return `You are ${futureAge} years old.` 
    if(birthYear > futureYear) return `You will be born in ${willBeBorn} years.`
}
